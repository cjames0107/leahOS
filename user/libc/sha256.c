/* SHA-256, moved out of the kernel along with the account database it served.
 *
 * The code is the kernel's, unchanged in substance: the same rounds, the same
 * salted-and-stretched password hash, the same comparison that does not leak
 * where two digests first differ. What changed is who runs it. Hashing a
 * password is arithmetic, and arithmetic has never needed ring 0.
 */
...
static uint32_t rotate_right(uint32_t value, uint32_t bits) { return value >> bits | value << (32 - bits); }
...
static void compress(uint32_t state[8], const uint8_t block[64])
{
    ...
    for (uint32_t i = 0; i < 64; ++i) {
        ...
        const uint32_t t1 = h + s1 + choose + kRoundConstants[i] + w[i];
                   ...
                   uint8_t out[SHA256_DIGEST])
{
    uint8_t buffer[256] = {};
    size_t n = 0;
    for (size_t i = 0; salt[i] != '\0' && n < sizeof(buffer); ++i)
        buffer[n++] = (uint8_t)(salt[i]);
    for (size_t i = 0; password[i] != '\0' && n < sizeof(buffer); ++i)
        buffer[n++] = (uint8_t)(password[i]);

    sha256(buffer, n, out);
    for (uint32_t i = 1; i < iterations; ++i)
        sha256(out, SHA256_DIGEST, out);
}

int digest_equal(const uint8_t* a, const uint8_t* b)
{
    uint8_t difference = 0;
    for (size_t i = 0; i < SHA256_DIGEST; ++i)
